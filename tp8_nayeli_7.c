...
int main(int argc, char *argv[]) {
	...
	// Encontrar las tres primeras cuentas con el mayor depósito
	int indices[3] = {-1, -1, -1}; // Inicializar los índices con valores inválidos
	for (i = 0; i < 100; i++) {
		for (j = 0; j < 3; j++) {
			if (indices[j] == -1 || cuentas[i][1] > cuentas[indices[j]][1]) {
				// Encontrado un depósito más grande que uno de los anteriores
				// Mover los índices correspondientes
				for (int k = 2; k > j; k--) {
